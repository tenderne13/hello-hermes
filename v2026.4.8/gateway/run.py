"""
Gateway 运行时模块 - 消息平台集成入口点。

提供：
- start_gateway(): 启动所有已配置的平台适配器
- GatewayRunner: 管理网关生命周期的核心类
...
# ---------------------------------------------------------------------------
# SSL 证书自动检测 — 用于 NixOS 及无法将 CA 证书暴露给 Python 的系统。
# 必须在导入任何 HTTP 库（discord、aiohttp 等）之前运行。
# ---------------------------------------------------------------------------
...
# 解析 Hermes 主目录（尊重 HERMES_HOME 覆盖）
...
from dotenv import load_dotenv  # 向后兼容：部分测试会 monkeypatch 此符号
from hermes_cli.env_loader import load_hermes_dotenv
_env_path = _hermes_home / '.env'
load_hermes_dotenv(hermes_home=_hermes_home, project_env=Path(__file__).resolve().parents[1] / '.env')

# 将 config.yaml 的值桥接到环境变量，以便 os.getenv() 获取。
# config.yaml 是终端设置的权威来源 — 优先于 .env。
...
if _config_path.exists():
    try:
        ...
        # config.yaml 对这些设置优先于 .env，因为这是文档化的配置路径。
        ...
        # 压缩配置由 run_agent.py 和 auxiliary_client.py 直接从 config.yaml 读取 — 无需桥接环境变量。
        # 辅助模型/直连端点覆盖（vision、web_extract）。
        ...
        if _agent_cfg and isinstance(_agent_cfg, dict):
            ...
            # 将 agent.gateway_timeout 桥接到 HERMES_AGENT_TIMEOUT 环境变量。
            # .env 中的环境变量优先（已在 os.environ 中）。
            if "gateway_timeout" in _agent_cfg and "HERMES_AGENT_TIMEOUT" not in os.environ:
                os.environ["HERMES_AGENT_TIMEOUT"] = str(_agent_cfg["gateway_timeout"])
        # 时区：将 config.yaml → HERMES_TIMEZONE 环境变量。
        # .env 中的 HERMES_TIMEZONE 优先（已在 os.environ 中）。
        ...
        pass  # 非致命错误；gateway 仍可使用 .env 值运行

# 尽早验证配置结构 — 记录警告以便网关操作员发现问题
try:
...
# 网关以静默模式运行 - 抑制调试输出并直接使用 cwd（无临时目录）
os.environ["HERMES_QUIET"] = "1"

# 在消息平台上启用交互式执行审批（危险命令）
os.environ["HERMES_EXEC_ASK"] = "1"

# 设置消息平台的终端工作目录。
# 如果用户在 config.yaml 中设置了显式路径（非 "." 或 "auto"），
# 尊重该设置。否则使用 MESSAGING_CWD 或默认为主目录。
...
def _normalize_whatsapp_identifier(value: str) -> str:
    """将 WhatsApp JID/LID 语法规范化为稳定的数字标识符。"""
    return (
        str(value or "")
        .strip()
        .replace("+", "", 1)
...
# 哨兵在会话开始处理时立即放入 _running_agents
# 处理，在任何 await 之前。防止同一会话的第二条消息
# 在 async gap 期间绕过"已运行"守卫，导致
# 守卫检查和实际代理创建之间出现重复代理。
...
def _check_unavailable_skill(command_name: str) -> str | None:
    """检查命令是否匹配已知但未激活的 skill。

    如果 skill 存在但已禁用或仅为可选安装，返回一条有帮助的消息。
    未找到匹配则返回 None。
    """
    # 规范化：命令使用连字符，skill 名称可能使用连字符或下划线
...
# ============================================================
# 【文档锚点 1C】GatewayRunner - 主网关控制器
# 职责：管理所有平台适配器的生命周期并在 agent 之间路由消息
# 入口：start_gateway() 函数
# 关键设计：AIAgent 实例缓存以保留 prompt 缓存
# ============================================================
class GatewayRunner:
    ...
    def __init__(self, config: Optional[GatewayConfig] = None):
        ...
        # 从 config.yaml / env vars 加载临时配置。
        # 两者仅在 API 调用时注入，绝不持久化。
        ...
        # 缓存每个会话的 AIAgent 实例以保留提示缓存。
        # 没有这个，每个消息都会创建新的 AIAgent，重建
        # 系统提示（含内存）每个回合都重建 — 破坏前缀缓存
        # 并在有提示缓存的提供商（Anthropic）上多花约 10 倍费用。
        # 键：session_key，值：(AIAgent, config_signature_str)
        import threading as _threading
        self._agent_cache: Dict[str, tuple] = {}
        self._agent_cache_lock = _threading.Lock()

        # 当主模型被限速时跟踪活动的备用模型/提供商。
        # 在触发备用模型的代理运行后设置；当主模型再次成功或用户通过 /model 切换时清除。
        ...
        # 键：session_key，值：当提示在等待用户输入时为 True。
        self._update_prompt_pending: Dict[str, bool] = {}

        # 以网关会话键为键的持久化 Honcho 管理器。
        # 这在短命的
        # 每消息 AIAgent 实例中保留 write_frequency="session" 语义。
    ...
    def _flush_memories_for_session(
        ...
        try:
            ...
            # 格式（"anthropic/claude-opus-4.6"），当
            ...
            tmp_agent = AIAgent(
                **runtime_kwargs,
                ...
                max_iterations=8,
                quiet_mode=True,
                skip_memory=True,  # 刷新代理 — 无内存提供商
                enabled_toolsets=["memory", "skills"],
                session_id=old_session_id,
            )
            # 完全静默刷新代理 — quiet_mode 仅抑制初始化
            # 消息；工具调用输出仍然通过
            # _safe_print → _print_fn 泄露到终端。设置空操作来防止。
    ...
    def _resolve_turn_agent_config(self, user_message: str, model: str, runtime_kwargs: dict) -> dict:
        ...
        # 【文档锚点 2B】Gateway 外壳与 CLI 一样，也在 turn 进入前执行动态模型路由
    ...
    async def _handle_adapter_fatal_error(self, adapter: BasePlatformAdapter) -> None:
        ...
        if not self.adapters and not self._failed_platforms:
            ...
            # 所有平台都已关闭并排队等待后台重连。
            # 如果错误可重试则以失败退出，以便 systemd Restart=on-failure
            # 可以重启进程。否则保持运行并在后台继续重试。
            if adapter.fatal_error_retryable:
    ...
    async def _session_expiry_watcher(self, interval: int = 300):
        ...
        """
        await asyncio.sleep(60)  # 初始延迟 — 让 gateway 完全启动
    ...
    def _create_adapter(
        ...
        if platform == Platform.TELEGRAM:
            ...
            adapter.gateway_runner = self  # 用于跨平台投递
            return adapter
    ...
    def _is_user_authorized(self, source: SessionSource) -> bool:
        ...
        检查顺序：
        1. 每个平台的 allow-all 标志（如 DISCORD_ALLOW_ALL_USERS=true）
        2. 环境变量允许列表（TELEGRAM_ALLOWED_USERS 等）
        3. DM 配对批准列表
        4. 全局 allow-all（GATEWAY_ALLOW_ALL_USERS=true）
        5. 默认：拒绝
        """
        # Home Assistant 事件是系统生成的（状态变化），不是
        # 用户发起的消息。HASS_TOKEN 已经认证了
        # 连接，因此 HA 事件始终已授权。
        # Webhook 事件通过适配器本身的 HMAC 签名验证进行认证 — 不适用用户允许列表。
        ...
        # 每个平台的 allow-all 标志（如 DISCORD_ALLOW_ALL_USERS=true）
        platform_allow_all_var = platform_allow_all_map.get(source.platform, "")
        if platform_allow_all_var and os.getenv(platform_allow_all_var, "").lower() in ("true", "1", "yes"):
            return True

        # 检查配对存储（始终检查，无论允许列表如何）
        ...
        # WhatsApp：从桥接会话映射文件解析 phone↔LID 别名
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        if not self._is_user_authorized(source):
            ...
            # 在 DM 中：提供配对码。在群组中：静默忽略。
            if source.chat_type == "dm" and self._get_unauthorized_dm_behavior(source.platform) == "pair":
                platform_name = source.platform.value if source.platform else "unknown"
                # 对所有配对响应（码或拒绝）进行速率限制，
                # 以防止当多个 DM 快速到达时向用户重复发送消息。
        ...
        # 更新进程（分离的）写了 .update_prompt.json；监视器
        # 将其转发给用户；现在用户的回复通过
        # .update_response 返回以便更新进程继续。
        ...
        # 以最小延迟处理。
        #
        # 特殊情况：Telegram/照片突发通常以多个近乎
        # 同时的更新形式到达。此处不要中断纯照片的后续消息；
        # 让适配器级别的批处理/队列逻辑吸收它们。

        # 过期驱逐：检测挂起/崩溃处理程序的泄漏锁。
        # 有了基于不活动的超时，活动任务可以运行数小时，
        # 所以仅靠挂钟时间是不够的。仅在代理
        # 空闲时间超过不活动阈值时（或当代理
        # 对象没有活动跟踪器且挂钟时间极长时）才驱逐。
        ...
        if _quick_key in self._running_agents and _stale_ts:
            ...
            # 永不驱逐待处理哨兵 — 它刚刚在异步设置阶段
            # 放置，在真实代理创建之前。哨兵没有
            # get_activity_summary()，所以下面的空闲检查
            # 总是会评估为 inf >= timeout 并立即
            # 驱逐它们，与设置路径竞争。
            _stale_idle = float("inf")  # 如果无法检查则假定为空闲
            _stale_detail = ""
        ...
        if _quick_key in self._running_agents:
            ...
            # — 执行器线程被阻塞，从不检查
            # _interrupt_requested。强制清理 _running_agents 以便会话
            # 解锁，后续消息正常处理。
            ...
            # /reset 和 /new 必须绕过运行代理守卫，以便它们
            # 实际作为命令调度，而不是作为用户
            # 文本排队（这会导致相同的
            # 损坏的历史 — #2170）。首先中断代理，然后
            # 清除适配器的待处理队列，以免陈旧的 "/reset" 文本
            # 在中断完成后作为用户消息重新处理。
            ...
            # /approve 和 /deny 必须绕过运行代理的中断路径。
            # 代理线程在 tools/approval.py 内的 threading.Event 上阻塞 —
            # 发送中断不会解除阻塞。直接路由到审批处理程序以便事件被发出信号。
        ...
        # 为任何识别的斜杠命令发送 command:* 钩子。
        # GATEWAY_KNOWN_COMMANDS 来自中央 COMMAND_REGISTRY
        # 在 hermes_cli/commands.py 中 — 此处无需维护硬编码集。
        ...
        if command:
            try:
                ...
                # 将下划线规范化为连字符，以便 Telegram 的带下划线
                # 自动补全表单与用连字符的命令配合工作。
                # 参见 hermes_cli/commands.py:_build_telegram_menu。
        ...
        # 技能斜杠命令：/skill-name 加载技能并发送到代理。
        # resolve_skill_command_key() 处理 Telegram 下划线/连字符
        # 往返，所以 Telegram 自动补全的 /claude_code 仍然解析
        # 为 claude-code 技能。
        if command:
            try:
                ...
                if cmd_key is not None:
                    ...
                    if _unavail_msg:
                        return _unavail_msg
                    # 真正未识别的 /command：不是内置、不是
                    # 插件、不是技能、不是已知的非活动技能。警告
                    # 用户而不是静默转发到 LLM
                    # 作为自由文本（这会导致静默失败行为）
                    # 如模型捏造 delegate_task 调用）。
                    # 在检查已知内置之前规范化为连字符形式
        ...
        # 待处理的执行批准由上述 /approve 和 /deny 命令处理。
        # 不要裸文本匹配 — 正常对话中的 "yes" 不得触发
        # 危险命令的执行。

        # ── 在任何 await 之前声明此会话 ───────────────────────
        # 在这里和 _run_agent 注册真实 AIAgent 之间，有
        # 多个 await 点（钩子、视觉增强、STT、
        # 会话卫生压缩）。没有这个哨兵，第二条
        # 消息在任何这些 yield 期间到达都会通过
        # "已运行" 守卫，并为
        # 同一会话启动重复代理 — 破坏记录。
        ...
        try:
            ...
            # 如果 _run_agent 用真实代理替换了哨兵，
            # 然后清理了它，这是一个空操作。如果我们提前退出
            # （异常、命令穿透等），哨兵必须
            # 不能停留，否则会话将被永久锁定。
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
        ...
        # 如果之前的会话已过期并被自动重置，添加通知
        # 以便代理知道这是新的对话（不是故意的 /reset）。
        ...
        # 为 DM topic 绑定自动加载技能（如 Telegram 私有聊天话题）
        # 仅在新会话上注入 — 对于正在进行的对话，技能内容
        # 已从第一条消息的会话历史中存在。
        ...
        if history and len(history) >= 4:
            ...
            # 从 config.yaml 读取模型 + 压缩配置。
            # 注意：卫生阈值故意设置得比代理的
            # 自己的压缩器（0.85 vs 0.50）更高。卫生是安全网，
            # 用于回合之间增长过大的会话 — 它在代理
            # 运行前触发以防止 API 失败。代理自己的压缩器处理
            # 在其工具循环中通过准确的
            # 实际令牌计数进行正常的上下文管理。卫生设置为 0.50 会导致
            # 长网关会话的每个回合都过早压缩。
            ...
            try:
                ...
                if _hyg_cfg_path.exists():
                    ...
                    # 读取压缩设置 — 仅使用启用标志。
                    # 阈值故意与代理的
                    # compression.threshold（卫生运行更高）分开。
                ...
                # 如果配置中没有，则从运行时解析 provider/base_url
                ...
                # 检查每个模型的自定义提供商 context_length
                # （与 run_agent.py 第 1171-1189 行相同的备用）。
            ...
            if _hyg_compression_enabled:
                ...
                # 优先使用上一回合的实际 API 报告令牌
                # （存储在会话条目中），而不是粗略的基于字符的估计。
                ...
                if _stored_tokens > 0:
                    ...
                    # 注意：粗略估计对代码/JSON 重的
                    # 会话高估 30-50%，但这只是意味着卫生检查稍早触发 —
                    # 这是安全无害的。85% 阈值已经提供了充足的
                    # 空间（代理自己的压缩器运行在 50%）。之前的 1.4x
                    # 倍增器试图通过膨胀阈值来补偿，但
                    # 85% * 1.4 = 119% 的上下文 — 这超过模型的限制
                    # 并阻止卫生检查为 ~200K 模型（GLM-5）触发。
                    ...
                    try:
                        ...
                        if _hyg_runtime.get("api_key"):
                            ...
                            if len(_hyg_msgs) >= 4:
                                ...
                                # 新的 session_id。将压缩的消息写入
                                # 新会话以便旧 transcript 保持完整
                                # 并可通过 session_search 搜索。
        ...
        # 如果此平台未设置家庭频道，则一次性提示
        # 跳过 Webhook — 它们直接传递到配置的目标 (github_comment, etc.)
        ...
        # 自动分析用户发送的图片
        #
        # 如果用户附加了图片，我们急切地运行视觉工具，以便
        # 对话模型总是收到文本描述。
        # 本地文件路径也包含在内，以便模型可以重新检查
        # 稍后通过 vision_analyze 用更有针对性的问题重新检查。
        #
        # 我们仅过滤图片路径（按 media_type），以便非图片
        # 附件（文档、音频等）不会发送到视觉
        # 工具，即使它们出现在同一条消息中。
        # -----------------------------------------------------------------
        ...
        #
        # 当多个用户共享单个线程会话（默认）时
        # ，在每条消息前加上【发送者名称】，以便代理能够
        # 区分参与者。跳过 DM（本质上是单用户）和
        # 当明确启用每用户线程隔离时。
        # -----------------------------------------------------------------
        _is_shared_thread = (
            source.chat_type != "dm"
        ...
        if event.media_urls and event.message_type == MessageType.DOCUMENT:
            ...
            for i, path in enumerate(event.media_urls):
                ...
                if not mtype.startswith(("application/", "text/")):
                    continue
                # 通过剥离 doc_{uuid12}_ 前缀提取显示文件名
                import os as _os
                basename = _os.path.basename(path)
                # 格式：doc_<12hex>_<original_filename>
        ...
        # -----------------------------------------------------------------
        # 当用户回复不在历史中的消息时注入回复上下文。
        # Telegram（和其他平台）让用户回复特定消息，
        # 但如果引用的消息来自之前的会话、cron 传递、
        # 或后台任务，代理没有关于正在引用什么的上下文。
        # 引用。在前面加上引用文本以便代理理解。（#1594）
        # -----------------------------------------------------------------
        ...
        try:
            ...
            # 如果代理的 session_id 在压缩期间改变，更新
            # session_entry，以便下面的记录写入正确的会话。
            ...
            # 检查待处理进程监视器（后台进程的 check_interval）
            try:
                from tools.process_registry import process_registry
                while process_registry.pending_watchers:
                    watcher = process_registry.pending_watchers.pop(0)
            ...
            # 阻塞网关审批机制内联处理。代理
            # 线程阻塞直到用户使用 /approve 或 /deny 响应，因此
            # 到达此处时审批已解决。旧的 post-loop pop_pending +
            # approval_hint 代码被移除，以支持
            # 镜像 CLI 同步 input() 的阻塞方法。
            
            # 保存完整对话到记录，包括工具调用。
            # 这保留了完整的代理循环（tool_calls、工具结果、
            # 中间推理），以便会话可以用完整上下文恢复，
            # 记录对调试和训练数据有用。
            #
            # 重要：当代理在生成任何响应之前失败时
            # （如上下文溢出 400），不要持久化用户的消息。
            # 持久化它会使会话更大，导致
            # 下一次尝试时出现相同失败 — 无限循环。（#1630）
            agent_failed_early = (
            ...
            # 如果这是一个新的会话（没有历史），写入完整工具
            # 定义作为第一个条目，以便记录是自描述的
            # — 与 API 请求中发送的 tools=[...] 相同的字典列表。
            if agent_failed_early:
                pass  # 跳过所有 transcript 写入 — 不要增长损坏的会话
            elif not history:
            ...
            # 仅找此回合的新消息（跳过我们加载的历史）。
            # 使用实际传递给代理的过滤历史长度（history_offset），
            # 而不是包含 session_meta
            # 条目的 len(history)，这些在代理看到之前被剥离了。
            if not agent_failed_early:
                ...
                if not new_messages:
                    ...
                    # 代理已经通过 _flush_messages_to_session_db() 将这些消息持久化到
                    # SQLite，所以此处跳过 DB 写入以防止
                    # 重复写入 bug (#860)。我们仍然写入
                    # JSONL 以保持向后兼容性并作为备份。
            ...
            # 令牌计数和模型现在由代理直接持久化。
            # 在这里仅保留 last_prompt_tokens 以便进行上下文窗口跟踪和
            # 压缩决策。
            ...
            # 如果流式已经传递了响应，提取并
            # 在返回 None 之前传递任何 MEDIA: 文件。流式
            # 发送包含 MEDIA: 标签的原始文本块 — 正常的
            # _process_message_background 中的后处理被跳过
            # 当 already_sent 为 True 时，所以媒体文件永远不会
            # 被传递，如果没有这个。
    ...
    async def _handle_reset_command(self, event: MessageEvent) -> str:
        ...
        # 清除任何会话范围的模型覆盖，以便下一个代理拾取
        # 配置的默认值而不是之前切换的模型。
        self._session_model_overrides.pop(session_key, None)

        # 发送插件 on_session_finalize 钩子（会话边界）
        try:
            from hermes_cli.plugins import invoke_hook as _invoke_hook
    ...
    async def _handle_profile_command(self, event: MessageEvent) -> str:
        ...
        # 从 HERMES_HOME 路径检测配置文件名
        # 配置文件路径类似：~/.hermes/profiles/<name>
    ...
    async def _handle_voice_channel_input(
        ...
        # guild_id，_send_voice_reply() 在语音频道播放音频。
    ...
    async def _send_voice_reply(self, event: MessageEvent, text: str) -> None:
        ...
        try:
            ...
            # 使用 .mp3 扩展名，以便 edge-tts 到 opus 的转换正常工作。
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        # YAML 1.1 将裸露的 "off" 解析为布尔值 False — 规范化回来
    ...
    async def _handle_compress_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            # _compress_context 已在旧会话上调用 end_session()
            # （在 SQLite 中保留其完整记录）并创建新的
            # session_id 以继续。将压缩的消息写入
            # 新会话，以便原始历史保持可搜索。
    ...
    async def _handle_title_command(self, event: MessageEvent) -> str:
        ...
        # 确保会话在 SQLite DB 中存在（它可能仅存在于 session_store）
        # 如果这是新会话中的第一个命令）
    ...
    # 允许 /update 的平台。ACP、API 服务器和 Webhook 是
    # 不应触发系统更新的编程接口。
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        # 生成 `hermes update --gateway` 分离，以便在网关重启中存活。
        # --gateway 为交互式提示启用基于文件的 IPC（stash
        # 恢复、配置迁移），以便网关可以将它们转发到
        # 用户，而不是静默跳过它们。
        # 使用 setsid 进行可移植会话分离（适用于系统服务）
        # 其中 systemd-run --user 由于缺少 D-Bus 会话而失败）。
    ...
    def _agent_config_signature(
        ...
        # 指纹化完整凭证字符串，而不是使用短的
        # 前缀。OAuth/JWT 风格的令牌经常共享共同前缀
        # （如 "eyJhbGci"），这可能导致跨认证的假缓存命中
        # 如果只考虑前几个字符。
        ...
        blob = _j.dumps(
            [
                ...
                # reasoning_config 排除 — 它在每条消息上设置在
                # 缓存代理上，不影响系统提示或工具。
                ephemeral_prompt or "",
            ],
            sort_keys=True,
    ...
    async def _run_agent(
        ...
        # 回退到环境变量以保持向后兼容。
        # YAML 1.1 将裸露的 `off` 解析为布尔值 False — 规范化
        # `or` 链之前，以免静默贯穿到 "all"。
        ...
        last_tool = [None]  # 闭包中跟踪的可变容器
        last_progress_msg = [None]  # 跟踪最后消息以进行去重
        repeat_count = [0]  # 同一消息重复多少次
        ...
        def progress_callback(event_type: str, tool_name: str = None, preview: str = None, args: dict = None, **kwargs):
            ...
            # 仅对 tool.started 事件采取行动（忽略 tool.completed、reasoning.available 等）
            ...
            # 去重：折叠连续相同的进度消息。
            # 与 execute_code 常见，其中模型用相同
            # 代码迭代（相同的样板导入 → 相同的预览）。
            if msg == last_progress_msg[0]:
                repeat_count[0] += 1
                # 用计数器更新 progress_lines 的最后一行
                # 通过特殊的 "dedup" 队列消息。
        ...
        #
        # 线程元数据是平台特定的：
        # - Slack DM 线程需要 event_message_id 备用（回复线程）
        # - Telegram 仅对论坛主题使用 message_thread_id；传递
        #   正常 DM/群组消息 id 作为 thread_id 会导致发送失败
        # - 其他平台应仅使用显式 source.thread_id
        ...
        async def send_progress_messages():
            ...
            progress_msg_id = None   # 要编辑的进度消息的 ID
            can_edit = True          # 一旦编辑失败则为 False（平台不支持）
            _last_edit_ts = 0.0      # 节流编辑以避免 Telegram 洪水控制
        ...
        # 桥接同步 step_callback → 异步 hooks.emit 用于 agent:step 事件
        _loop_for_step = asyncio.get_event_loop()
        _hooks_ref = self.hooks

        def _step_callback_sync(iteration: int, prev_tools: list) -> None:
            try:
                # prev_tools 可以是 list[str] 或带 "name"/"result" 的 list[dict]
                # 键。规范化以保持 "tool_names" 向后兼容，
        ...
        # 桥接同步 status_callback → 异步 adapter.send 用于上下文压力
        ...
        def run_sync():
            # 下面 `message` 的条件重新赋值
            # （预先添加模型切换注释）使 Python 将其视为
            # 整个函数中的局部变量。`nonlocal` 让我们
            # 读取 *和* 重新赋值外部 `_run_agent` 参数而不
            # 在早期读取时触发 UnboundLocalError
            # `_resolve_turn_agent_config(message, …)`。
            nonlocal message

            # 通过 env var 传递 session_key 到进程注册表，以便后台
            # 进程可以映射回此网关会话
            os.environ["HERMES_SESSION_KEY"] = session_key or ""

            # 从 env var 读取或使用默认值（与 CLI 相同）
            max_iterations = int(os.getenv("HERMES_MAX_ITERATIONS", "90"))
            ...
            # 将平台上下文与用户配置的临时系统提示组合
            combined_ephemeral = context_prompt or ""
            if self._ephemeral_system_prompt:
                combined_ephemeral = (combined_ephemeral + "\n\n" + self._ephemeral_system_prompt).strip()

            # 重新读取 .env 和配置以获取新凭证（网关寿命长，
            # 密钥可能在不重启的情况下更改）。
            try:
            ...
            # 将历史转换为代理格式。
            # 两种情况：
            #   1. 正常路径（来自记录）：简单 {role, content, timestamp} 字典
            #      - 剥离时间戳，保留 role+content
            #   2. 中断路径（来自代理结果["messages"]）：完整代理消息
            #      可能包含 tool_calls、tool_call_id、reasoning 等。
            #      - 这些必须完整传递，以便 API 看到有效
            #        assistant→tool 序列（删除 tool_calls 会导致 500 错误）
            agent_history = []
            for msg in history:
                role = msg.get("role")
                if not role:
                ...
                # 丰富的代理消息（tool_calls、工具结果）必须被传递
                # 完整传递，以便 API 看到有效的 assistant→tool 序列
            ...
            # 收集历史中已有的 MEDIA 路径，以便我们可以排除它们
            # 从当前回合的提取。这是压缩安全的：
            # 即使消息列表缩小，我们也知道哪些路径是旧的。
            ...
            # 注册每会话网关批准回调，以便危险
            # 命令批准阻止代理线程（镜像 CLI input()）。
            # 回调桥接 sync→async 以发送批准请求
            # 立即向用户。
            from tools.approval import (
            ...
            def _approval_notify_sync(approval_data: dict) -> None:
                ...
                """
                # 代理等待时暂停打字指示器
                # 用户批准。对于 Slack 的 Assistant API 至关重要，
                # assistant_threads_setStatus 禁用组合框 — 
                # 用户在"is thinking..."时实际上无法输入 /approve
                # 活动时。批准消息发送自动清除 Slack
                # 状态；暂停防止 _keep_typing 重新设置它。
            ...
            # 扫描工具结果中需要传递的 MEDIA:<path> 标签
            # 作为原生音频/文件附件。TTS 工具嵌入 MEDIA: 标签
            # 在其 JSON 响应中，但模型的最终文本回复通常
            # 不包含它们。我们从工具结果中收集唯一标签并
            # 附加任何尚不在最终响应中的标签，以便
            # 适配器的 extract_media() 可以找到并精确传递文件一次。
            #
            # 使用基于路径的去重与 _history_media_paths（收集
            # 在 run_conversation 之前）而不是索引切片。即使
            ...
            # 同步 session_id：代理可能在此期间创建了新会话
            # 运行中上下文压缩（_compress_context 分割会话）。
            ...
            # 当压缩创建新会话时，消息列表被
            # 缩短了。使用原始历史偏移会产生
            # 空的 new_messages 切片，导致网关仅写入
            # user/assistant 对 — 丢失压缩摘要和尾部。
            # 重置为 0，以便网关写入所有压缩消息。
        ...
        async def monitor_for_interrupt():
            ...
            while True:
                await asyncio.sleep(0.2)  # 每 200ms 检查
                # 检查适配器是否有此会话的待处理中断。
                # 必须使用 session_key（build_session_key 输出）— 不是
        ...
        # 长时间运行任务的定期"仍在工作"通知。
        # 每 10 分钟触发一次，以便用户知道代理没有死。
        _NOTIFY_INTERVAL = 600  # 10 分钟
        ...
        try:
            # 在线程池中运行以不阻塞。使用基于 *不活动* 的
            # 超时而不是挂钟限制：代理可以运行
            # 数小时，如果它正在积极调用工具/接收流令牌，
            # 但挂起的 API 调用或没有活动的卡住工具
            # 在配置持续时间内被捕获并杀死。（#4815）
            #
            # 配置：agent.gateway_timeout 在 config.yaml 中，或
            # HERMES_AGENT_TIMEOUT 环境变量（环境变量优先）。
            ...
            if _agent_timeout is None:
                ...
                # 轮询循环：检查代理的内置活动跟踪器
                # （在每次工具调用、API
                # 调用和流增量上由 _touch_activity() 更新）每几秒。
                response = None
                while True:
            ...
            # 检查我们是否被中断或有排队消息（/queue）。
            result = result_holder[0]
            adapter = self.adapters.get(source.platform)
            
            # 从适配器获取待处理消息。
            # 使用 session_key（不是 source.chat_id）来匹配适配器的存储键。
            pending = None
            ...
            # "/new"），丢弃它 — 命令不应传递给代理
            # 作为用户输入。主要修复在 base.py 中（命令绕过）
            # 活动会话守卫），但这捕获了命令
            # 通过 interrupt_message 回退泄漏的边缘情况。
            ...
            if pending:
                ...
                # 清除适配器的中断事件，以便下一个 _run_agent 调用
                # 在新代理甚至发出第一个 API 调用之前不会立即重新触发中断
                #（这导致了无限循环）。
                if adapter and hasattr(adapter, '_active_sessions') and session_key and session_key in adapter._active_sessions:
                    adapter._active_sessions[session_key].clear()
                
                # 限制递归深度以防止在
                # 用户在代理持续失败时发送多条消息时资源耗尽。（#816）
...
# ============================================================
# start_gateway() - 网关启动入口
# 流程：双实例保护 -> 同步skills -> 初始化日志 -> 启动Runner -> 写PID -> 启动cron -> 等待关闭
# 返回：成功True，失败False（导致非零退出码以便systemd自动重启）
# ============================================================
