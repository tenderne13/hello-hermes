...
用法:
    hermes                     # 交互式聊天 (默认)
    hermes chat                # 交互式聊天
    hermes gateway             # 前台运行gateway
    hermes gateway start       # 启动gateway服务
    hermes gateway stop        # 停止gateway服务
    hermes gateway status      # 查看gateway状态
    hermes gateway install     # 安装gateway服务
    hermes gateway uninstall   # 卸载gateway服务
    hermes setup               # 交互式设置向导
    hermes logout              # 清除存储的认证
    hermes status              # 显示所有组件状态
    hermes cron                # 管理cron任务
    hermes cron list           # 列出cron任务
    hermes cron status         # 检查cron调度器是否运行
    hermes doctor              # 检查配置和依赖
    hermes honcho setup                    # 配置Honcho AI记忆集成
    hermes honcho status                   # 显示Honcho配置和连接状态
    hermes honcho sessions                 # 列出目录→会话名映射
    hermes honcho map <name>               # 将当前目录映射到会话名
    hermes honcho peer                     # 显示对等名称和辩证设置
    hermes honcho peer --user NAME         # 设置用户对等名称
    hermes honcho peer --ai NAME           # 设置AI对等名称
    hermes honcho peer --reasoning LEVEL   # 设置辩证推理级别
    hermes honcho mode                     # 显示当前记忆模式
    hermes honcho mode [hybrid|honcho|local]  # 设置记忆模式
    hermes honcho tokens                   # 显示token预算设置
    hermes honcho tokens --context N       # 设置session.context() token上限
    hermes honcho tokens --dialectic N     # 设置辩证结果字符上限
    hermes honcho identity                 # 显示AI对等身份表示
    hermes honcho identity <file>          # 从文件(SOUL.md等)植入AI对等身份
    hermes honcho migrate                  # 分步迁移指南: OpenClaw native → Hermes + Honcho
    hermes version             显示版本
    hermes update              更新到最新版本
    hermes uninstall           卸载Hermes Agent
    hermes acp                 作为ACP服务器运行(编辑器集成)
    hermes sessions browse     交互式会话选择器(带搜索)

    hermes claw migrate --dry-run  # 预览迁移(不实际更改)
"""
...
def _require_tty(command_name: str) -> None:
    """如果 stdin 不是终端，则退出并显示明确错误。

    交互式 TUI 命令（hermes tools, hermes setup, hermes model）使用
    curses 或 input() 提示，当 stdin 是管道时，这些提示会导致 CPU 占用率达到 100%。
    此保护措施可防止意外的非交互式调用。
    """
...
# ---------------------------------------------------------------------------
# 配置文件覆盖 — 必须在任何hermes模块导入之前执行
# 许多模块在导入时缓存HERMES_HOME(模块级常量)
# 我们在此拦截--profile/-p并设置环境变量,使后续os.getenv("HERMES_HOME")正确解析
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """在模块导入前预解析 --profile/-p 参数并设置 HERMES_HOME。"""
    # 【文档锚点 1B】控制层：在导入其他 hermes 模块前先解析 profile，决定 HERMES_HOME
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. 如果没有标志,检查 ~/.hermes/active_profile
        ...
        try:
            ...
            if active_path.exists():
                ...
                if name and name != "default":
                    profile_name = name
                    consume = 0  # 不从argv中剥离任何内容
        except (UnicodeDecodeError, OSError):
            pass  # 文件损坏,跳过

    # 3. 如果找到了profile,解析并设置 HERMES_HOME
    if profile_name is not None:
        ...
        # 从argv中剥离标志,避免argparse解析失败
        if consume > 0:
            for i, arg in enumerate(argv):
                if arg in ("--profile", "-p"):
                    start = i + 1  # +1 因为 argv 是 sys.argv[1:]
...
# 加载 .env 文件: 优先 ~/.hermes/.env, 其次项目根目录(开发时)
from hermes_cli.config import get_hermes_home
from hermes_cli.env_loader import load_hermes_dotenv
load_hermes_dotenv(project_env=PROJECT_ROOT / '.env')

# 尽早初始化集中式文件日志 — 所有hermes子命令都写入agent.log + errors.log
try:
    from hermes_logging import setup_logging as _setup_logging
    _setup_logging(mode="cli")
except Exception:
    pass  # 尽力而为,日志设置失败不导致CLI崩溃
...
def _relative_time(ts) -> str:
    """将时间戳格式化为相对时间(例如'2h ago','yesterday')。"""
    if not ts:
        return "?"
    delta = _time.time() - ts
    if delta < 60:
...
def _has_any_provider_configured() -> bool:
    """检查是否至少配置了一个推理provider."""
    from hermes_cli.config import get_env_path, get_hermes_home, load_config
    from hermes_cli.auth import get_auth_status

    # 判断Hermes是否已明确配置(配置中的model不是硬编码默认值)
    # 用于控制外部工具凭证(Claude Code, Codex CLI)是否应跳过首次安装向导
    ...
    # 仅 OPENAI_BASE_URL 也算数 — 本地模型(vLLM, llama.cpp 等)通常不需要 API key.
    ...
    # 检查特定 provider 的认证回退(例如通过 gh auth 的 Copilot)。
    try:
    ...
    # 说明用户已完成配置(全新安装时 model 是普通字符串)。
    # 同时覆盖在 config 而非 .env 中存储 api_key/base_url 的自定义端点。
    if isinstance(model_cfg, dict):
    ...
    # 只有在 Hermes 已明确配置时才计算 — Claude Code 已安装
    # 并不意味着用户想让 Hermes 使用他们的 token。
...
def _session_browse_picker(sessions: list) -> Optional[str]:
    """基于curses的交互式会话浏览器,支持实时搜索过滤

    返回选中的会话ID,取消则返回None
    使用curses(而非simple_term_menu)以避免tmux/iTerm中方向键的ghost-duplication渲染bug
    """
    if not sessions:
    ...
    try:
        ...
        def _format_row(s, max_x):
            ...
            # 根据终端宽度自适应列宽
            # 布局: [箭头 3] [标题/预览 弹性] [活跃时间 12] [来源 6] [ID 18]
            fixed_cols = 3 + 12 + 6 + 18 + 6  # 箭头 + 活跃时间 + 来源 + ID + 填充
        ...
        def _curses_browse(stdscr):
            ...
            if curses.has_colors():
                ...
                curses.init_pair(1, curses.COLOR_GREEN, -1)   # 选中项
                curses.init_pair(2, curses.COLOR_YELLOW, -1)  # 标题
                curses.init_pair(3, curses.COLOR_CYAN, -1)    # 搜索
                curses.init_pair(4, 8, -1)                    # 暗淡
            ...
            while True:
                ...
                # 计算可见区域
                visible_rows = max_y - 4  # 标题 + 列标题 + 空白 + 页脚
                if visible_rows < 1:
                    visible_rows = 1
...
def _resolve_last_cli_session() -> Optional[str]:
    """从SQLite查找最近的CLI会话ID,不可用则返回None."""
    try:
...
def _resolve_session_by_name_or_id(name_or_id: str) -> Optional[str]:
    """将会话名(title)或ID解析为会话ID

    - 如果看起来像会话ID(含下划线+十六进制),优先直接查找
    - 否则当作title处理,使用resolve_session_by_title(自动最新)
    - 第一个方法匹配失败时回退到另一个方法
    """
    try:
        from hermes_state import SessionDB
...
# ============================================================================
# cmd_chat() - CLI启动链第3步: 绑定到chat子命令,组装参数并调用cli_main()
# ============================================================================
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if last_id:
                ...
                print("未找到可继续的上一个 CLI 会话。")
                sys.exit(1)

    # 如果不是直接的会话 ID，则通过 title 解析 --resume
    ...
    # ----------------------------------------------------------
    # 步骤2: 首次运行检查 - 确保已配置 API provider
    # ----------------------------------------------------------
    if not _has_any_provider_configured():
        print()
        print("看起来 Hermes 尚未配置 -- 未找到 API key 或 provider。")
        print()
    ...
    # ----------------------------------------------------------
    # 步骤4: 同步 bundled skills (快速, 跳过未更改的内容)
    # ----------------------------------------------------------
    try:
    ...
    # --source: 标记会话来源(如 'tool' 用于第三方集成)
    if getattr(args, "source", None):
        os.environ["HERMES_SESSION_SOURCE"] = args.source

    # ----------------------------------------------------------
    # 步骤5: 导入并调用 cli.py 中的 cli_main()
    # 这是 CLI 启动链的最后一环: HermesCLI.__init__() → _init_agent() → AIAgent
    # ----------------------------------------------------------
    from cli import main as cli_main

    # 从 args 构建 kwargs 参数字典
    kwargs = {
        "model": args.model,
    ...
    try:
        cli_main(**kwargs)  # 【文档锚点 1B】控制权从 hermes_cli.main 移交给 CLI 外壳
...
def select_provider_and_model(args=None):
    ...
    # 以与 CLI 启动时相同的方式读取有效的 provider:
    # config.yaml model.provider > 环境变量 > 自动检测
    import os
    ...
    try:
        ...
        try:
            ...
            active = None  # 尚无 provider;默认为列表中的第一个
    ...
    # 步骤1: Provider 选择 — 主要 provider 显示在前,其余在"更多..."后面
    top_providers = [
    ...
    # 从 config.yaml 添加用户定义的自定义 provider
    custom_providers_cfg = config.get("custom_providers") or []
    _custom_provider_map = {}  # key → {名称, base_url, api_key}
...
def _model_flow_openrouter(config, current_model=""):
    ...
    if selected:
        ...
        # 更新配置的 provider 并停用任何 OAuth provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    if not state or not state.get("access_token"):
        ...
        try:
            ...
            return
        # login_nous 已处理模型选择 + 配置更新
        return

    # 已登录 — 使用精选模型列表(与 OpenRouter 默认值相同)。
    # 实时 /models 端点返回数百个模型; 精选列表
    # 只显示用户熟悉的 OpenRouter 代理模型。
    ...
    # 对于两个层级: 首先应用白名单过滤器(移除非白名单
    # 免费模型和白名单中实际上并不免费的模型)。
    # 然后对于免费用户: 将剩余模型分区为可选/不可用。
    ...
    # 解析升级链接的门户 URL(在 staging 环境可能不同)
    _nous_portal_url = ""
    try:
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # 优先使用凭证池(`hermes auth` 存储 device_code token 的地方),
    # 回退到旧的 provider 状态。
    try:
...
def _model_flow_custom(config):
    ...
    if model_name:
        ...
        model.pop("api_mode", None)  # 让运行时从 URL 自动检测
        save_config(cfg)
        deactivate_provider()

        # 同步调用者的配置字典,以便设置向导最后的
        # save_config(config) 保留我们的模型设置。如果没有
        # 这个,向导会用自己配置字典中的陈旧值覆盖 model.provider/base_url (#4172)。
        ...
        if base_url or api_key:
            deactivate_provider()
        # 即使没有模型名称,也将自定义端点持久化到
        # 调用者的配置字典中,以便设置向导不会丢失它。
    ...
    # 自动保存到 custom_providers,以便下次显示在菜单中
...
def _save_custom_provider(base_url, api_key="", model="", context_length=None):
    ...
    按 base_url 去重 — 如果 URL 已存在,更新
    模型名称和 context_length,但不会添加重复条目。
    从 URL 主机名自动生成显示名称。
    """
    from hermes_cli.config import load_config, save_config
...
def _model_flow_named_custom(config, provider_info):
    ...
    # 将模型名称保存到 custom_providers 条目中以便下次使用
...
# 直接 API-key provider 的精选模型列表 — 单个来源在 models.py
...
def _model_flow_kimi(config, current_model=""):
    ...
    # 步骤3: 模型选择 — 显示适合该端点的模型
    if is_coding_plan:
        # Coding Plan 模型(kimi-for-coding 优先)
        model_list = [
            "kimi-for-coding",
            "kimi-k2.5",
            "kimi-k2-thinking",
        ...
        # 旧版 Moonshot 模型(不包括仅 Coding Plan 的模型)
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    # 模型选择 — 解析顺序:
    #   1. models.dev 注册表(缓存,过滤了代理/工具型模型)
    #   2. 精选静态回退列表(离线保险)
    #   3. 实时 /models 端点探测(没有 models.dev 数据的小型 provider)
    curated = _PROVIDER_MODELS.get(provider_id, [])

    # 优先尝试 models.dev — 返回工具型模型,过滤了噪音
    mdev_models: list = []
    try:
    ...
    if selected:
        ...
        # 用 provider、base URL 和特定于 provider 的 API 模式更新配置
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # 用 provider 更新配置 — 清除 base_url,因为
        # resolve_runtime_provider() 总是硬编码 Anthropic 的 URL。
        # 在配置中保留过时的 base_url 会污染其他
        # provider,如果用户切换时不运行 'hermes model'。
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def _update_via_zip(args):
    """通过下载 ZIP 压缩包更新 Hermes Agent。

    用于 Windows 平台：当 git 文件 I/O 因防病毒软件、NTFS 过滤驱动等问题
    导致 'Invalid argument' 错误时，改为 ZIP 方式更新。
    """
    import shutil
    import tempfile
    import zipfile
    ...
    try:
        ...
        # GitHub ZIP 解压后目录名为 hermes-agent-<branch>/
        ...
        # 将更新文件复制到现有安装目录，保留 venv/node_modules/.git
    ...
    # 重新安装 Python 依赖。优先安装 .[all]，但如果某个可选依赖
    # 在此机器上安装失败，则保留基础依赖并逐个重新安装其余可选依赖，
    # 以免更新静默剥离已有功能。
    ...
    if uv_bin:
        ...
        # 使用 sys.executable 显式调用 venv 的 pip 模块，
        # 避免 Debian/Ubuntu 上的 PEP 668 'externally-managed-environment' 错误。
        # 部分环境下 venv 内的 pip 会丢失；先用 ensurepip 引导恢复，
        # 再尝试可编辑模式安装。
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    # 如果索引有未合并条目（如中断的 merge/rebase 导致），
    # git stash 会失败并提示 "needs merge / could not write index"。
    # 用 `git reset` 清除冲突状态以继续 stash。工作树
    # 的更改会保留；只会清除索引中的冲突标记。
...
def _restore_stashed_changes(
    ...
    # 检查是否有未合并（冲突）的文件 — 即使 returncode 为 0 也可能发生
    unmerged = subprocess.run(
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # 非交互模式（gateway /update）不中止 — 代码更新本身已成功，
        # 只是 stash 恢复时发生冲突。中止会错误地报告整个更新失败。
        if prompt_user:
            sys.exit(1)
        return False
...
def _get_origin_url(git_cmd: list[str], cwd: Path) -> Optional[str]:
    """获取 origin remote 的 URL，未设置则返回 None。"""
    try:
        result = subprocess.run(
...
def _is_fork(origin_url: Optional[str]) -> bool:
    """检查 origin remote 是否指向 fork（而非官方仓库）。"""
    if not origin_url:
        return False
    # 规范化 URL 以便比较（去除末尾的 .git）
...
def _add_upstream_remote(git_cmd: list[str], cwd: Path) -> bool:
    """将官方仓库添加为 'upstream' remote。成功返回 True。"""
    try:
        result = subprocess.run(
...
def _count_commits_between(git_cmd: list[str], cwd: Path, base: str, head: str) -> int:
    """统计 `head` 上不在 `base` 中的 commit 数。出错返回 -1。"""
    try:
        result = subprocess.run(
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    实现逻辑：
    - 如果 upstream remote 不存在，询问用户是否添加
    - 比较 origin/main 与 upstream/main
    - 如果 origin/main 完全落后于 upstream/main，则从 upstream 拉取
    - 可能的话尝试将 fork 同步回 origin
    """
    ...
    # 如果 origin/main 有不在 upstream 上的 commit，不要覆盖
    if origin_ahead > 0:
        print()
    ...
    # 如果 upstream 没有领先，fork 已同步
    if upstream_ahead == 0:
        print("  ✓ Fork is up to date with upstream")
        return

    # origin/main 完全落后于 upstream/main（可快进）
    print()
...
def _invalidate_update_cache():
    """删除所有 profile 的更新检查缓存，以免横幅错误报告
    成功更新后的过期 "commits behind" 数量。

    git 仓库在 profile 间共享 — 当一个 profile 运行
    ``hermes update`` 后，所有 profile 都已更新。
    """
    homes = []
    # 默认 profile 主目录
    default_home = Path.home() / ".hermes"
    homes.append(default_home)
    # 命名的 profile 在 ~/.hermes/profiles/ 下
...
def _load_installable_optional_extras() -> list[str]:
    ...
    仅逐个重试 ``[all]`` 实际包含的可选依赖。
    ``[all]`` 之外的依赖（如 ``rl``、``yc-bench``）故意排除 —
    它们有重型或平台特定依赖，大多数用户从未安装。
    """
    try:
        import tomllib
    ...
    # 解析 [all] 组以查找它引用的依赖。
    # 条目格式如 "hermes-agent[matrix]" 或 "package-name[extra]"。
...
def cmd_update(args):
    ...
    # gateway 模式：用基于文件的 IPC 而非 stdin 传递提示
    gw_input_fn = (lambda prompt, default="": _gateway_prompt(prompt, default)) if gateway_mode else None
    
    print("⚕ Updating Hermes Agent...")
    print()
    
    # 优先尝试 git 方式更新，Windows 下当 git 文件 I/O 损坏时
    # 回退到 ZIP 下载（防病毒软件、NTFS 过滤驱动等）
    ...
    # Windows 上 git 可能因文件系统原子性问题失败：
    # "unable to write loose object file: Invalid argument"。
    # 设置推荐的解决方案。
    ...
    # 构建 git 命令一次 — fork 检测和更新过程都复用它。
    git_cmd = ["git"]
    if sys.platform == "win32":
        git_cmd = ["git", "-c", "windows.appendAtomically=false"]

    # 检测是否从 fork 更新（在任何分支逻辑之前）
    ...
    try:
        ...
        try:
            ...
            if pull_result.returncode != 0:
                # ff-only 失败 — 本地和远程已分叉（如 upstream
                # force-push 或 rebase）。由于本地更改已 stash，
                # 重置到与远程完全一致。
        ...
        # 清理过时的 .pyc 字节码缓存 — 防止 gateway
        # 重启时因更新后的源码引用旧字节码中不存在的名称而 ImportError
        # （如 hermes_constants 新增了 get_hermes_home）。
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")

        # Fork 上游同步逻辑（仅在 fork 的 main 分支上执行）
        ...
        # git pull 后，磁盘上的源文件比此进程中的缓存 Python
        # 模块更新。重新加载 hermes_constants，使下方 lazy
        # 导入（如 skills 同步、gateway 重启）能看到新增的
        # 属性如 display_hermes_home()。
        try:
            import importlib
        ...
        # 同步捆绑的 skills（复制新的、更新已修改的、保留用户删除的）
        try:
            from tools.skills_sync import sync_skills
            print()
            print("→ Syncing bundled skills...")
        ...
        try:
            ...
            pass  # profiles 模块不可用或没有 profile

        # 同步 Honcho host 块到所有 profile
        try:
        ...
        if needs_migration:
            ...
            if response in ('', 'y', 'yes'):
                print()
                # gateway 模式仅运行自动迁移（不用 input() 提示
                # 输入 API key，否则会让分离进程挂起）。
        ...
        # 更新后自动重启所有 gateway。
        # 代码更新（git pull）在所有 profile 间共享，因此每个
        # 运行的 gateway 都需要重启以加载新代码。
        try:
            ...
            # --- Systemd 服务 (Linux) ---
            # 发现所有 hermes-gateway* 单元（默认 + profile）
            if is_linux():
                try:
                ...
                for scope, scope_cmd in [("user", ["systemctl", "--user"]), ("system", ["systemctl"])]:
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if not parts:
                                continue
                            unit = parts[0]  # 如 hermes-gateway.service 或 hermes-gateway-coder.service
            ...
            # --- 手动（非服务）gateway ---
            # 杀死不由服务管理的剩余 gateway 进程。
            # 排除属于刚重启服务的 PID，以免
            # 立即杀死 systemd/launchd 刚生成的进程。
...
def _coalesce_session_name_args(argv: list) -> list:
    """将 -c/--continue 和 -r/--resume 后未加引号的多词会话名合并。

    当用户输入 ``hermes -c Pokemon Agent Dev`` 而未给会话名加引号时，
    argparse 会将其视为三个独立 token。本函数将其合并为单个参数，
    使 argparse 收到 ``['-c', 'Pokemon Agent Dev']``。

    在 flag 后收集 token，直到遇到另一个 flag（``-*``）
    或已知顶级子命令。
    """
    _SUBCOMMANDS = {
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        try:
            ...
            # 为新 profile 自动克隆 Honcho 配置（仅在 --clone/--clone-all 时）
            if clone or clone_all:
                try:
            ...
            # 播种捆绑的 skills（如果 --clone-all 已复制则跳过）
            if not clone_all:
...
# ============================================================================
# 【文档锚点 1B】main() - CLI 启动链第 2 步: 参数解析,将 chat 子命令绑定到 cmd_chat()
# 入口: hermes 脚本 → hermes_cli.main:main() → cmd_chat() → cli.main() → HermesCLI
# ============================================================================
def main():
    """Hermes CLI主入口点."""
...
""",
    ...
    # 解析参数并执行
    # =========================================================================
    # 预处理argv: 将-c/-r后面的多词会话名合并为单个token
    ...
    # 顶级 --resume / --continue 作为chat的快捷方式
...
# ============================================================================
# CLI启动链入口
# hermes脚本 → hermes_cli.main:main() → cmd_chat() → cli.main() → HermesCLI
# ============================================================================
